...
void end_of_clothoid (double x_i, double y_i, 
  ...
  if ( fabs (sigma) < Epsilon ) // sigma nul
    ...
    /* s: length, t: theta, k(s): curvature, sig: sigma
     * k(s) = k0 + sig*s
     *
     * ds*2*pi/(2*pi/k(s)) = dt => dt/ds = k(s)
     * =>
     * t(s) = k0*s + 1/2*sig*s^2
     */
     ...
     *
     * =>
     * x(s) = int(cos(1/2*sig*u^2 + k0*u), u=0..s);
     * y(s) = int(sin(1/2*sig*u^2 + k0*u), u=0..s);
     */
